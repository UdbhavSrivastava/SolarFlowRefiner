...
def load_training_module(path: Path):
    spec = importlib.util.spec_from_file_location("flowrefiner_ode_train", path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load FlowRefiner-ODE training module from {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["flowrefiner_ode_train"] = module
    spec.loader.exec_module(module)
    return module


refiner = load_training_module(THIS_DIR / "train.py")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate FlowMatch -> FlowRefiner model.")
    parser.add_argument("--test-index", type=Path, required=True)
    parser.add_argument("--test-base-cache", type=Path, required=True, help="Cached normalized FlowMatch residuals for test split.")
...
def main() -> int:
    ...
    sample_steps = int(args.sample_steps if args.sample_steps is not None else ckpt_args.get("sample_steps", 8))
    solver = args.solver or ckpt_args.get("solver", "heun")

    ds, _meta, _channels = refiner.build_dataset(
        args.test_index,
        metadata,
        stats,
    ...
    metrics = refiner.evaluate(model, loader, stats, sample_steps, solver, device, args.ssim_data_range)
    record = {
